...
function Report() {
    ...
    const categories = places.points_of_interest ? Object.keys(places.points_of_interest) : null;
    const custom_addresses = places.custom_addresses ? Object.entries(places.custom_addresses) : null;
    const custom_objects = places.custom_addresses ? Object.entries(places.custom_objects) : null;
    logger.log(custom_objects);

    const [selectedCategoryPreferences, setselectedCategoryPreferences] = useState(null);
    const [selectedAddressPreferences, setselectedAddressPreferences] = useState(false);
    const [selectedObjectPreferences, setselectedObjectPreferences] = useState(false);
    ...
    const allObjects = Object.values(custom_objects).flatMap(([categoryName, categoryList], index1) => {
        return Object.entries(categoryList).flatMap(([subcategoryName, subcategory]) => {
    ...
    const [allPreferences, setAllPreferences] = useState(() => {
        ...
        return places.points_of_interest ? Object.values(places.points_of_interest).reduce((acc, category) => {
        ...
        <div className="report">
            <div className="reportContainer">
                ...
                <div className='reportMain'>
                    <div className={categories && categories.length <= 9 ? 'leftReport-short' : 'leftReport'}>
                    { custom_addresses && custom_addresses.length > 0 ? (
                    <div>
                            <div className={selectedAddressPreferences === true ? 'categoryNameSelected' : 'categoryName'} onClick={() => handleAddressClick()}>
                                <label className='categoryLabel'>{t('Your locations')}</label>
                            </div>
                            <div className={selectedAddressPreferences === true ? 'show-data-hr-place-selected' : 'show-data-hr-place'}>
                    ...
                    <div>
                            <div className={selectedObjectPreferences === true ? 'categoryNameSelected' : 'categoryName'} onClick={() => handleObjectClick()}>
                                <label className='categoryLabel'>{t('Your places')}</label>
                            </div>
                            <div className={selectedObjectPreferences === true ? 'show-data-hr-place-selected' : 'show-data-hr-place'}>
                                <hr className='show-data-search-place-hr'/>
                            </div>
                    </div>   
                    ) : null}
                    ...
                    <div className='rightReport'>
                        ...
                        {selectedAddressPreferences && Object.entries(custom_addresses).map((address, index) => {
                                    ...
                                    <div className='preferenceItem'>
                                        <div className='mainItemData'>
                                            ...
                                            <div className='addressDistances'>
                                                <div className='preferenceAddressItemDistance'>
                                                    <div className='icon'>
                                                        <Icon icon="material-symbols-light:directions-car-outline" id='person-simple-walk-light'/>
                                                    </div>
                                                    <div className='distance-time'>
                                                        <label className='distance'>
                                                            {address_info.commute_time.bike.distance > 1000 ? 
                                                                `${(address_info.commute_time.drive.distance / 1000).toFixed(1)} km` : 
                                                                `${Math.ceil(address_info.commute_time.drive.distance / 10) * 10} m`}
                                                        </label>
                                                        <label className='time'>{Math.ceil(address_info.commute_time.drive.duration / 60)} min</label>
                                                    </div>
                                                </div>
                                                <div className='preferenceAddressItemDistance'>
                                                    <div className='icon'>
                                                        <Icon icon="material-symbols-light:directions-bus-outline" id='person-simple-walk-light'/>
                                                    </div>
                                                    <div className='distance-time'>
                                                        <label className='distance'>
                                                            {address_info.commute_time.transit.distance > 1000 ? 
                                                                `${(address_info.commute_time.transit.distance / 1000).toFixed(1)} km` : 
                                                                `${Math.ceil(address_info.commute_time.transit.distance / 10) * 10} m`}
                                                        </label>
                                                        <label className='time'>{Math.ceil(address_info.commute_time.transit.duration / 60)} min</label>
                                                    </div>
                                                </div>
                                                <div className='preferenceAddressItemDistance'>
                                                    <div className='icon'>
                                                    ...
                                                    <div className='distance-time'>
                                                        <label className='distance'>
                                                            {address_info.commute_time.walk.distance > 1000 ? 
                                                                `${(address_info.commute_time.walk.distance / 1000).toFixed(1)} km` : 
                                                                `${Math.ceil(address_info.commute_time.walk.distance / 10) * 10} m`}
                                                        </label>
                                                        <label className='time'>{Math.ceil(address_info.commute_time.walk.duration / 60)} min</label>
                                                    </div>
                                                </div>
                                                <div className='preferenceAddressItemDistance'>
                                                    <div className='icon'>
                                                        <Icon icon="ph:person-simple-bike-light" id='person-simple-walk-light'/>
                                                    </div>
                                                    <div className='distance-time'>
                                                        <label className='distance'>
                                                            {address_info.commute_time.bike.distance > 1000 ? 
                                                                `${(address_info.commute_time.bike.distance / 1000).toFixed(1)} km` : 
                                                                `${Math.ceil(address_info.commute_time.bike.distance / 10) * 10} m`}
                                                        </label>
                                                        <label className='time'>{Math.ceil(address_info.commute_time.bike.duration / 60)} min</label>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        {selectedObjectPreferences && allObjects.map((object, index) => {
                            return(
                                <div className='preferenceItem' key={index}>
                                        ...
                                        <div className='preferenceItemDistance'>
                                            ...
                                            <div className='distance-time'>
                                                ...
                                                <label className='time'>{Math.ceil(object.distance / 83)} min</label>
                                            </div>
                                        </div>
                                    </div>
                            );
                        })}
                        {!selectedAddressPreferences && !selectedObjectPreferences && selectedCategoryPreferences && Object.keys(selectedCategoryPreferences).map((category, index) => {
                            const preference = allPreferences.find(pref => pref.key === category);
